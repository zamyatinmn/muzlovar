...
| `прослушиваний`, `прослушиваний_альбома`, `прослушиваний_артиста`, `номер_трека`, `номер_диска`, `битрейт`, `битовая_глубина`, `частота_дискретизации`, `темп`, `каналы`, `размер`, `размер_альбома`, `треков_в_альбоме` | 0 | — | 1 |
| `длительность`, `длительность_альбома` | 0 | — | — |
| `год`, `оригинальный_год`, `год_релиза`, `библиотека`, `средняя_оценка`, `replaygain` и остальные поля ReplayGain | — | — | — |

**Ошибки** (блокируют компиляцию):

* значение вне границ: `оценка = 7`, `прослушиваний >= -1`;
