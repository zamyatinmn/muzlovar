...
* **список** — все `.mix`/`.nsp`: название, описание, файлы,
  ...
  и дата изменения;
* **конструктор** — метаданные, палитра полей, дерево условий
  (drag & drop и кнопки-альтернативы для клавиатуры), сортировка,
  лимит, живой предпросмотр канонического `.mix` и содержимого
  файлов на диске;
...
* `GET /api/schema` — единая схема полей и операторов (источник —
  Haskell-модель, собственных копий списков в JavaScript нет);
...
> в интерфейсе (раздел «Метаданные», значок «личное» у условия) — и
> здесь тоже.
